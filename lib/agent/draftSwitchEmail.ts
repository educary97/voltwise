// src/lib/agent/draftSwitchEmail.ts
// Uses Claude to draft a switching request email to the new supplier
...
export interface DraftedEmail {
  to: string;       // supplier email address
  subject: string;
...
3. Request that the supplier handles the termination of the existing contract (as required by ERSE rules)
4. Be polite and professional
5. Be in European Portuguese

Also provide a suitable email subject line.
...
Respond ONLY with a JSON object in this exact format (no markdown, no preamble):
{
  ...
  const supplierEmail =
    bestOffer.supplierEmail ?? `clientes@${bestOffer.supplier.toLowerCase().replace(/\s/g, "")}.pt`;
