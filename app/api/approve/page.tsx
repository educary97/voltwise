...
// The review page — shown when you tap the WhatsApp approval link
// Displays the comparison summary + full email draft, then sends on confirm
...
function ApprovePageInner() {
  ...
  // Decode payload client-side for display (signature verified server-side on confirm)
  ...
  if (!data || !sig) {
    return <ErrorState message="Link inválido. Por favor verifica o WhatsApp e tenta novamente." />;
  }

  if (decodeError) {
    return <ErrorState message={decodeError} />;
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0a0a0f", color: "#e8e8f0" }}>
      ...
      >
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 16, letterSpacing: "-0.02em" }}>Voltwise</div>
        ...
        {status === "sent" ? (
          ...
          <>
            ...
            <div style={{ marginBottom: 32 }}>
              ...
              >
                Oportunidade de poupança encontrada
              </h1>
              <p style={{ color: "#888", fontSize: 14, marginBottom: 24 }}>
                O agente Voltwise analisou o mercado e encontrou uma oferta melhor. Revê o resumo e
                confirma para enviar o pedido de mudança.
              </p>
              ...
              >
                <Card
                  label="Plano atual"
                  value={`€${Number(comparison.currentMonthlyCost).toFixed(2)}/mês`}
                  sub={`Fornecedor atual`}
                  accent="#ef4444"
                />
                <Card
                  label={comparison.bestSupplier}
                  value={`€${Number(comparison.estimatedMonthlyEur).toFixed(2)}/mês`}
                  sub={comparison.bestPlan}
                  accent="#22c55e"
                />
              </div>
              ...
              <div
                style={{
                  background: "linear-gradient(135deg, #0d2e1a 0%, #0a2010 100%)",
            ...
            {/* Email draft */}
            <div style={{ marginBottom: 32 }}>
              <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: 16, color: "#aaa" }}>
                📧 RASCUNHO DO EMAIL
              </h2>
            ...
            <p style={{ fontSize: 12, color: "#444", marginTop: 16, textAlign: "center" }}>
              Ao confirmar, o email será enviado em teu nome para {email.to}. O novo fornecedor
              trata da rescisão do contrato atual.
            </p>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ textAlign: "center", padding: "60px 0" }}>
      ...
      <p style={{ color: "#888", fontSize: 15, lineHeight: 1.6 }}>
        O pedido de mudança foi enviado para <strong style={{ color: "#f5c842" }}>{sentTo}</strong>
        .<br />
        O {supplier} irá tratar da rescisão do teu contrato atual.
        <br />
